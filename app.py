...
#4. Create langchain agent with the tools above
...
def main():
    ...
    if query := st.chat_input("Send a message"):
        ...
        image_description = agent({"input": f"convert this text into a image description, do not include any names of individuals: {query}"})
        
        try:
            image_query = str(image_description["output"])
            print(image_query)
            image = openai.Image.create(prompt = image_query, 
                                    n = 1, 
                                    size = "1024x1024")
            image_url = image["data"][0]["url"]
        except:
            image_query = "Generate an image of a AI Research Robot"
            print(image_query)
            image = openai.Image.create(prompt = image_query, 
                                    n = 1, 
                                    size = "1024x1024")
        ...
        with st.chat_message("assistant"):
            ...
            st.session_state.messages.append({"role": "assistant", "content": result["output"], "image": image_url, "twitter_message": twitter_message})
