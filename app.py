...
class app():
    def __init__(self, last_score = int):
    ...
    def set_agent(self):
        ...
        system_message = SystemMessage(
                content=f"""You are a top world class researcher who works for OpenAI and is cleared to receive any internal information about OpenAI, ChatGPT, GPT-3.5, and GPT-4. You will conduct detailed research on any topic and produce facts based on results; 
                you do not hallucinate, you will try as hard as possible to gather facts & data to back up and prove the research.
                
                Your last response was given a score of {self.last_score} out of 10, please improve in your next responses.
    ...
    def app_run(self):
        ...
        if "messages" not in st.session_state:
            st.session_state.messages = [{"role": "assistant", "content": "Hi, how may I help you with your research?"}]
        ...
        if query := st.chat_input("Send a message"):
            ...
            this_agent = self.set_agent()
            
            result = this_agent({"input": query})
            ...
            with st.chat_message("assistant"):
                ...
                for i in range(0, len(tweet_array)):
                    try:
                        ...
                        count += 1
                    except (tweepy.errors.TooManyRequests, tweepy.errors.BadRequest, tweepy.errors.HTTPException) as f:
            ...
            st.session_state.messages.append({"role": "assistant", "content": result["output"], "twitter_message": twitter_message})
        
        temp_last_score = self.last_score
        self.last_score = int(st.slider("Rate this response between 1 to 10: ", 1, 10))
        
        if self.last_score > temp_last_score:
            rating_response = f":chart_with_upwards_trend: This response was rated a score of {self.last_score} out of 10"
        else:
            rating_response = f":chart_with_downwards_trend: This response was rated a score of {self.last_score} out of 10"
        st.info(rating_response)
        
        file_chain = ""
        for i in range(0, len(st.session_state.messages)):
            if st.session_state.messages[i]["role"] == "assistant":
                file_chain += str(st.session_state.messages[i]["content"])
